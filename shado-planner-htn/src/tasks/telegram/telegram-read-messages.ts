...
import type { TelegramMessage } from '@shado-network/client-telegram'
import type { HtnTask, TriggerProps } from '../../types/planner.js'
...
export default {
  ...
  conditions: {
    ...
    'telegram-has-messages': (props) =>
      props._origin.memory.state?.['telegram-has-messages'] === true,
  ...
  actions: {
    'telegram-read-messages': async (props) => {
      try {
        ...
        props._origin.memory.state?.['telegram-messages'].forEach(
          async (message: TelegramMessage) => {
            ...
            if (
              !props._origin.clients['telegram']
                .getMessageThreads()
            ...
            // NOTE: Send the reply.
            await props._origin.clients['telegram'].replyToMessage(
              response as string,
...
const _shouldReplyToMessage = (
  ...
  //       handle ===
  //       process.env[
  //         `TELEGRAM_${props._origin.id.toUpperCase()}_BOT_HANDLE`
  //       ]
  //     )
  //   })
